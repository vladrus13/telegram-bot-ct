...
class Updates {
    ...
    companion object {
        ...
        private val pairs: List<Pair<String, SendMessage.() -> Unit>> = listOf(
            Pair("1.0") {
                text =
                    "Обновление! Теперь для каждой таблицы будет подписано время обновления! Вы всегда сможете посмотреть, насколько стара таблица"
            },
            Pair("1.1") {
                text =
                    "Обновление! Теперь при выборе группы вы автоматически будете выходить из папки выбора групп! Идея: @staszw"
            }
        )
